...
static struct UIMenuItem MainMenuItems[] = {
    ...
    {MITEM_BREAK},
    {MITEM_BREAK},
    {MITEM_BUTTON, MAIN_MENU_ID_BTN_FORMAT_MC, MITEM_FLAG_POS_MID, 0, 24, 0, 0, SYS_UI_LBL_FORMAT_MC},
    {MITEM_BREAK},
    {MITEM_BREAK},
...
static struct UIMenuItem MCMenuItems[] = {
    ...
    {MITEM_BREAK},
    {MITEM_BREAK},
    {MITEM_BUTTON, MAIN_MENU_ID_BTN_DUMP_MC, MITEM_FLAG_POS_MID, 0, 24, 0, 0, SYS_UI_LBL_DUMP_MC},
    {MITEM_BREAK},
    {MITEM_BREAK},
    {MITEM_BUTTON, MAIN_MENU_ID_BTN_REST_MC, MITEM_FLAG_POS_MID, 0, 24, 0, 0, SYS_UI_LBL_REST_MC},
    {MITEM_BREAK},
    {MITEM_BREAK},
...
static struct UIMenuItem ProgressScreenItems[] = {
    ...
    {MITEM_LABEL, PRG_SCREEN_ID_ETA_LBL, 0, 0, 0, 0, 0, SYS_UI_LBL_ETA},
    {MITEM_TAB},
    {MITEM_VALUE, PRG_SCREEN_ID_ETA_HOURS, MITEM_FLAG_READONLY, MITEM_FORMAT_UDEC, 2},
    {MITEM_COLON, PRG_SCREEN_ID_ETA_MINS_SEP},
    {MITEM_VALUE, PRG_SCREEN_ID_ETA_MINS, MITEM_FLAG_READONLY, MITEM_FORMAT_UDEC, 2},
    {MITEM_COLON, PRG_SCREEN_ID_ETA_SECS_SEP},
    {MITEM_VALUE, PRG_SCREEN_ID_ETA_SECS, MITEM_FLAG_READONLY, MITEM_FORMAT_UDEC, 2},
    {MITEM_BREAK},
    {MITEM_BREAK},
    {MITEM_LABEL, PRG_SCREEN_ID_RATE_LBL, 0, 0, 0, 0, 0, SYS_UI_LBL_RATE},
    {MITEM_TAB},
    {MITEM_TAB},
    {MITEM_VALUE, PRG_SCREEN_ID_RATE, MITEM_FLAG_READONLY},
    {MITEM_LABEL, PRG_SCREEN_ID_RATE_UNIT, 0, 0, 0, 0, 0, SYS_UI_LBL_KBPS},
    {MITEM_BREAK},
    {MITEM_BREAK},
...
static struct UIMenuItem InsuffSpaceScreenItems[] = {
    ...
    {MITEM_STRING, INSUFF_SPC_SCREEN_ID_MESSAGE, MITEM_FLAG_READONLY},
    {MITEM_BREAK},
    {MITEM_BREAK},

    {MITEM_LABEL, 0, 0, 0, 0, 0, 0, SYS_UI_LBL_AVAILABLE_SPC},
    {MITEM_TAB},
    {MITEM_VALUE, INSUFF_SPC_SCREEN_ID_AVAIL_SPC, MITEM_FLAG_READONLY, MITEM_FORMAT_UDEC},
    {MITEM_LABEL, INSUFF_SPC_SCREEN_ID_AVAIL_SPC_UNIT},
    {MITEM_BREAK},
    {MITEM_LABEL, 0, 0, 0, 0, 0, 0, SYS_UI_LBL_REQUIRED_SPC},
    {MITEM_TAB},
    {MITEM_TAB},
    {MITEM_VALUE, INSUFF_SPC_SCREEN_ID_REQD_SPC, MITEM_FLAG_READONLY, MITEM_FORMAT_UDEC},
    {MITEM_LABEL, INSUFF_SPC_SCREEN_ID_REQD_SPC_UNIT},
    {MITEM_BREAK},
...
static int SelectMemoryCardSlot(struct McData *McData, int nCards)
{
    int McPort;
    ...
    /* Always ask which slot to use (instead of burying Format behind R1 menus). */
    McPort = DisplayPromptMessage(SYS_UI_MSG_SELECT_SLOT, SYS_UI_LBL_SLOT1, SYS_UI_LBL_SLOT2);
    if (McPort == 0)
        return -1;
    McPort--;

    if (McData[McPort].Type != MC_TYPE_PS2) {
        DisplayErrorMessage(SYS_UI_MSG_NO_CARDS);
        return -1;
    }
...
static int FormatMemoryCardSlot(struct McData *McData, int McPort)
{
    int result;
...
void MainMenu(void)
{
    ...
    while (!done) {
        ...
        switch (event) {
            ...
            case EVENT_INSTALL:
                /* Install */
                McPort = SelectMemoryCardSlot(McData, McPort);
                if (McPort < 0)
                    break;
                ...
                /* Ask whether to format before installing (avoids digging through R1 menus). */
                result = DisplayPromptMessage(SYS_UI_MSG_FORMAT_BEFORE_INST, SYS_UI_LBL_NO, SYS_UI_LBL_YES);
                if (result == 0)
                    break;
                if (result == 2) {
                    if (FormatMemoryCardSlot(McData, McPort) < 0)
                        break;
                }
            ...
            case EVENT_FORMAT_HDD:
                if (DisplayPromptMessage(SYS_UI_MSG_FORMAT_HDD_MANUAL, SYS_UI_LBL_CANCEL, SYS_UI_LBL_OK) == 2) {
                    if (hddFormat() != 0)
                        DisplayErrorMessage(SYS_UI_MSG_FORMAT_HDD_FAILED);
                }
                break;
