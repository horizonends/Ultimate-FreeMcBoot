...
- File Manager: wLaunchELF_R3Z as `BOOT.ELF`
- Controller Tester, Cheat Device (NTSC/PAL), POPStarter
- Stock USB drivers in `SYS-CONF` (stable boot). EXFAT helpers under `FMCB_EXFAT/` only
...
## Notes
- Do not put EXFAT USB IRXes into main `SYS-CONF` (can hang at logo)
- Upstream FMCB/installer credits: see main project README
