...
ZIP_FEAT="${REPO}/UFMCB-Features-v1.0.3.zip"
ZIP_TOOL="${REPO}/UFMCB-Toolkit-v1.0.3.zip"
...
printf '%s\n' 'title=UFMCB Features v1.0.3' 'boot=ashorizonends_Installer_EXFAT.elf' > "$USB/UFMCB-Features/title.cfg"
printf '%s\n' 'title=UFMCB Toolkit v1.0.3' 'boot=ashorizonends_Installer_EXFAT.elf' > "$USB/UFMCB-Toolkit/title.cfg"
...
rm -f "$USB"/UFMCB-Features-v1.0.3-alpha.*.zip "$USB"/UFMCB-Toolkit-v1.0.3-alpha.*.zip
rm -f "$USB"/UFMCB-Features-v1.0.3.zip "$USB"/UFMCB-Toolkit-v1.0.3.zip 2>/dev/null || true
# re-copy current zips below
